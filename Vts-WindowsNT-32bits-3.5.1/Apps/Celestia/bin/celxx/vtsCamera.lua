...
-- VERSION : 2.1 : FA : FA_56 : 23/07/2012 : Problème d'affichage de la caméra senseur à l'état initial
...
-- VERSION : 2.0 : FA : FA_34 : 23/07/2012 : FA - Scale système solaire crash quand on recule le point de vue.
...
--
-- VER : $Id: vtsCamera.lua 8612 2020-12-09 20:02:41Z mmo $
--------------------------------------------------------------------------------
...
function initSatellite( satName, parentPath )
   ...
   -- Couleur de l'ellipsoide de la covariance en position par defaut
   gCurrentSat.positionalCovarianceColor = "ffffff"
   
   -- Facteur d'échelle de l'ellipsoide de la covariance en position par defaut
   gCurrentSat.positionalCovarianceScale = 1.0

   -- Sous objets du satellite
   gCurrentSat.nodes = {}

   -- L'objet racine est le référenciel
   local rootObject = celestia:find( parentPath .. "/" .. satName .. "_ref" )
   ...
   for k, child in pairs(gCurrentSat.nodes) do
      ...
      -- Détection du suffixe _cov_ref pour ignorer les reference de covariance
      local posRef = string.find( child.object:name(), "_cov_ref" )
      if( posRef == nil ) then
   ...
   -- Stockage de l'arbre du satellite
...
function storeRadiusAndWalkChildren( node )
   ...
   for i, child in pairs(childrenList) do
      ...
      local posRef = string.find( child:name(), "_sens_ref" )
      if( posRef == nil ) then
